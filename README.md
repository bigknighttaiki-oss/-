...
Dropbox のアプリ登録をしなくても、サンプルデータで全画面を触れます。
クローンして Xcode で開き、そのまま実行して「サンプルデータで試す」を押すだけです。

- バックアップ画面: アップロードの進捗、リネーム・失敗・スキップを含む結果一覧、
  削除確認までひと通り流れます
- ストレージ画面: 走査中の表示から、種別ごとの内訳、容量の大きいファイル一覧まで出ます

デモ表示中は画面上端に帯が出ます。**この間はネットワークにもディスクにも触れません** —
削除確認で「ゴミ箱に移動」を押しても、実際のファイルは動きません。
帯の「デモを終了」で通常の状態に戻ります。
